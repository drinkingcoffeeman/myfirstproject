...
import com.hwkim.becarefulbang.utils.FBAuth
...
class ContentListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(category == "category1") {
            ...
            myRef = database.getReference("contents4") //컨텐츠 2인경우를 가져옴 리얼타임 DB에 있는것
        } else if(category == "category5") {
            myRef = database.getReference("contents5") //컨텐츠 2인경우를 가져옴 리얼타임 DB에 있는것
        } else if(category == "category6") {
            myRef = database.getReference("contents6") //컨텐츠 2인경우를 가져옴 리얼타임 DB에 있는것
        } else if(category == "category7") {
            myRef = database.getReference("contents7") //컨텐츠 2인경우를 가져옴 리얼타임 DB에 있는것
        } else if(category == "category8") {
            myRef = database.getReference("contents8") //컨텐츠 2인경우를 가져옴 리얼타임 DB에 있는것
